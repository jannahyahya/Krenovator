              ...
              <v-container>
                <v-row>
                  <v-col cols="12" sm="6" md="4">
                    <v-text-field
                      v-model="formdata.RFIDID"
                      label="RFIDID"
                    
                      required
                      @input="$v.RFIDID.$touch()"
                    >
                    </v-text-field>
                  </v-col>
                  <v-col cols="12" sm="6" md="4">
...
<script>
import { fetchAll, fetchUpdate, AddStudent ,DeleteStudent  } from "@/api/Student";
...
export default {
  data: () => ({
    ...
    dialogDelete: false,
    headers: [
      { text: "RFIDID", value: "RFIDID", sortable: false },
      { text: "Student_Name", value: "Student_Name", sortable: false },
      { text: "ClassID", value: "ClassID", sortable: false },
      { text: "Student_email", value: "Student_email", sortable: false },
      ...
      this.dialogDelete = true;
       ...
       let remove=item;
        ...
        this.tabledata.push(remove);
         DeleteStudent(remove).then(response => {
    ...
    save() {
      if (this.editedIndex > -1) {
        ...
        this.tabledata.push(data1);
         AddStudent(data1).then(response => {
          console.log(response);
        });
